...
function renderInstruction(instruction){const labels=instruction.labels.length?instruction.labels:['追加条件なし'];$('parsedInstruction').innerHTML=`<b>読み取った条件</b><div>${labels.map(label=>`<span>${esc(label)}</span>`).join('')}</div><small>無料の決定論的ルール解析です。認識できない文は勝手に制約へ変換しません。</small>`}

function renderMission(){const report=state.report;$('priorityCount').textContent=report.priority_count;$('candidateCount').textContent=report.candidates.length;$('pathCount').textContent=report.graphrag.paths.length;$('traceId').textContent=report.meta.trace_id.slice(0,8).toUpperCase();$('disclaimer').innerHTML=`<b>DEMO DATA</b> — ${esc(report.meta.disclaimer)} 候補・価格も架空データです。実データ接続後に意思決定へ使用してください。`;$('weaknessGrid').innerHTML=report.weaknesses.map(weaknessCard).join('');$('candidateList').innerHTML=report.candidates.map(candidateRow).join('')||'<p class="empty-ranking"><b>条件に合う候補がいません。</b><br>年齢・予算・役割のいずれかを緩めてください。</p>';renderInstruction(report.instruction);document.querySelectorAll('.candidate-row').forEach(row=>{const open=()=>showCandidate(report.candidates.find(item=>item.player_id===row.dataset.player));row.onclick=open;row.onkeydown=event=>{if(event.key==='Enter')open()}})}
...
async function loadReport(){const budget=$('budgetInput').value;const query=$('rankingInput').value.trim();$('budgetValue').textContent=budget;state.report=await getJSON(`/api/report?budget_m=${budget}&q=${encodeURIComponent(query)}`);renderMission();renderGraph();state.telemetry=await getJSON('/api/observability');renderSystem()}
...
$('rankingForm').addEventListener('submit',event=>{event.preventDefault();$('candidateList').innerHTML='<div class="skeleton"></div>';loadReport()});
document.querySelectorAll('[data-command]').forEach(button=>button.addEventListener('click',()=>{$('rankingInput').value=button.dataset.command;$('candidateList').innerHTML='<div class="skeleton"></div>';loadReport()}));
